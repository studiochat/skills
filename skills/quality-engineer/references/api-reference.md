...
| `category` | `"requerido"` \| `"prohibido"` | No | Judge modality. **Set it on every new text assertion** (see below). Omit only when editing legacy free-text criteria. |
...
- **`requerido`** (the assistant MUST do X). Covers stating info AND asking for data.
  ```json
  {"type": "text", "category": "requerido", "criteria": "mencionar el plazo máximo de acreditación para retiros en pesos"}
  {"type": "text", "category": "requerido", "criteria": "pedir el número de operación"}
  ```
- **`prohibido`** (the assistant must NOT do X).
  ```json
  {"type": "text", "category": "prohibido", "criteria": "usar emojis"}
  {"type": "text", "category": "prohibido", "criteria": "revelar el proveedor técnico de Pix"}
  ```
...
```json
{
  ...
  "assertions": [
    ...
    {"type": "text", "category": "requerido", "criteria": "confirmar el monto del reembolso en su respuesta"}
  ]
}
```
